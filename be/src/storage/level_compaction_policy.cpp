...
bool LevelCompactionPolicy::need_compaction() {
    ...
    LOG(INFO) << "need_compaction compaction context:" << _compaction_context->to_string();
...
std::shared_ptr<CompactionTask> LevelCompactionPolicy::create_compaction() {
    ...
    LOG(INFO) << "compaction context:" << _compaction_context->to_string();
...
// the first rowset in level-0 may be compacted already, and the creation_time may be large than
...
void LevelCompactionPolicy::_pick_level_0_rowsets(bool* has_delete_version, size_t* rowsets_compaction_score,
    ...
    for (auto rowset : _compaction_context->rowset_levels[0]) {
        ...
        // TODO: 有bug，如果rowset是刚刚被compaction的singleton，就会导致这个rowset的creation时间被改为最新的时间，导致延迟30s被调度
        // For level-0, should consider the rowset creation time
        // So the candidate tablet may be skipped
...
Status LevelCompactionPolicy::_check_version_continuity(const std::vector<RowsetSharedPtr>& rowsets) {
    ...
    for (int i = 0; i < rowsets.size() - 1; ++i) {
        ...
        if (next_rowset->start_version() != current_rowset->end_version() + 1) {
            ...
            return Status::InternalError("level compaction miss version error.");
        }
    }
...
std::shared_ptr<CompactionTask> LevelCompactionPolicy::_create_level_0_compaction() {
    ...
    LOG(INFO) << "pick input rowsets size:" << input_rowsets.size()
              << ", rowsets_compaction_score:" << rowsets_compaction_score
              << ", has_delete_version:" << has_delete_version;
    ...
    CompactionTaskFactory factory(output_version, _compaction_context->tablet, std::move(input_rowsets), 0);
...
std::shared_ptr<CompactionTask> LevelCompactionPolicy::_create_level_1_compaction() {
    ...
    if (input_rowsets.size() <= 1) {
        if (input_rowsets.size() == 1) {
            LOG(INFO) << "only one rowset:" << input_rowsets[0]->version();
        }
    ...
    if (input_rowsets.size() == 2 && input_rowsets[0]->empty()) {
        ...
        LOG(INFO) << "only two rowset. one is [0-1]. do not compact. tablet:"
    ...
    CompactionTaskFactory factory(output_version, _compaction_context->tablet, std::move(input_rowsets), 1);
    std::shared_ptr<CompactionTask> compaction_task = factory.create_compaction_task();
    return compaction_task;
}
