...
Status VerticalCompactionTask::run_impl() {
    // TODO:考虑metrics
    // 加锁，控制并发，与其他任务（schema change/clone）等的并发问题

    // 需要考虑compaction任务失败的时候，重新调度该compaction任务，并且需要设置失败的时间，调度的时候，需要考虑上次失败的时间
...
Status VerticalCompactionTask::_compact_column_group(bool is_key, int column_group_index,
    ...
    VLOG(1) << "tablet=" << _tablet->tablet_id() << ", column group=" << column_group_index
    ...
    OLAPStatus olap_status = output_rs_writer->flush_columns();
    if (olap_status != OLAP_SUCCESS) {
...
StatusOr<size_t> VerticalCompactionTask::_compact_data(bool is_key, int32_t chunk_size,
    ...
    while (!should_stop()) {
        ...
        OLAPStatus olap_status = output_rs_writer->add_columns(*chunk, column_group, is_key);
        if (olap_status != OLAP_SUCCESS) {
