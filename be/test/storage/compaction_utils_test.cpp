...
TEST(CompactionUtilsTest, test_algorithm_to_string) {
    ...
    const char* ag_1_name = algorithm_to_string(ag_1);
    EXPECT_STREQ("HORIZONTAL_COMPACTION", ag_1_name);

    CompactionAlgorithm ag_2 = VERTICAL_COMPACTION;
    const char* ag_2_name = algorithm_to_string(ag_2);
    EXPECT_STREQ("VERTICAL_COMPACTION", ag_2_name);

    const char* ag_3_name = algorithm_to_string((CompactionAlgorithm)2);
...
TEST(CompactionUtilsTest, test_choose_compaction_algorithm) {
    ...
    int64_t max_columns_per_group = 5;
    size_t source_num = 10;
    CompactionAlgorithm choice = CompactionUtils::choose_compaction_algorithm(num_columns, max_columns_per_group, source_num);
...
TEST(CompactionUtilsTest, test_create_compaction_policy) {
    CompactionContext context;
    std::unique_ptr<CompactionPolicy> policy = CompactionUtils::create_compaction_policy(&context);
